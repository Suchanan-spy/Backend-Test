...
class StudentSubjectsScoreAPIView(APIView):
    ...
    def post(request, *args, **kwargs):
        ...
        qureyset = StudentSubjectsScore.objects.filter(filed_1 = student_first_name, filed_2 = student_last_name, filed_3 = subjects_title, filed_4 = score)
        
        fristName = StudentSubjectsScore.objects.create(name='Frist Name')
        lastName = StudentSubjectsScore.objects.create(name='Last Name')
        subTitle = StudentSubjectsScore.objects.create(name='subjects title')
        scoree = StudentSubjectsScore.objects.create(name='Score')

        return Response(status=status.HTTP_201_CREATED)
    
    def custom_exception_handler(student_first_name, student_last_name, subjects_title, score):
        response = exception_handler(student_first_name, student_last_name, subjects_title, score)
...
class StudentSubjectsScoreDetailsAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        per = Personnel.objects.values_list("first name", "last name", )
        per2 = Personnel.objects.values("school class")
        scoreNumber = StudentSubjectsScore.objects.values("score")
        
        if(scoreNumber >=80 and scoreNumber <= 100):
            print("A")
        elif (scoreNumber >=75 and scoreNumber < 80):
            print("B+")
        elif (scoreNumber >=70 and scoreNumber < 75):
            print("B")
        elif (scoreNumber >=65 and scoreNumber < 70):
            print("C+")
        elif (scoreNumber >=60 and scoreNumber < 65):
            print("C")
        elif (scoreNumber >=55 and scoreNumber < 60):
            print("D+")
        elif (scoreNumber >=50 and scoreNumber < 55):
            print("D")
        elif (scoreNumber < 50):
            print("F")
        
        
        sub = StudentSubjectsScore.objects.filter(filed_1=Subjects, filed_2=credits)
...
class PersonnelDetailsAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        school_title = kwargs.get("Dorm Palace School", None)
        all_class = Personnel.objects.all().values
        your_result = []

        for class_call in all_class:
            if school_class == 'Dorm Palace School':
                callData = class_call. school: all_class['Dorm Palace School'], role: all_class['personnel type'], class: all_class['school class'], name: school_class[first name][last name] .   
                result_api = requests.post(apiDB, headers=hed,data=callData).json()
                your_result =  your_result + result_api['data']
            else:
                print("error")
